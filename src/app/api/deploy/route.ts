...
if (!NETLIFY_ACCESS_TOKEN) {
  throw new Error(
    "NETLIFY_ACCESS_TOKEN is not defined. Please set it as an environment variable."
  );
}
...
export async function POST(req: Request) {
  try {
    ...
    console.log(link_global);

    return Response.json({ data: link_global });
  } catch (error) {
